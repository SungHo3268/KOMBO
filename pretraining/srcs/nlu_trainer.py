...
from nlu_tasks.nlu_configs.KorNLI.data_utils import load_task_dataset as KorNLI_dataset
from nlu_tasks.nlu_configs.KorSTS.data_utils import load_task_dataset as KorSTS_dataset
from nlu_tasks.nlu_configs.NSMC.data_utils import load_task_dataset as NSMC_dataset
from nlu_tasks.nlu_configs.PAWS_X.data_utils import load_task_dataset as PAWS_X_dataset

from pretraining.srcs.functions import float_separator, BAR_FORMAT
from pretraining.srcs.nlu_utils import get_bert_tokenizer, get_task_model, get_optimizer, get_lr_scheduler
...
class Trainer(nn.Module):
    ...
    def get_input(self, dataset, shuffle: bool):
        ...
        for i in tqdm(range(len(labels)), desc="Getting inputs...", bar_format=BAR_FORMAT):
            ...
            # encoded_input = self.tokenizer(sentence1s[i], sentence2s[i], truncation=True, padding=True, return_tensors="pt")
    ...
    def _forward(self, inputs, labels):
        with amp.autocast():
            ...
            # logits, loss, seq_len = None, None, None
    ...
    def fine_tuning(self):
        ...
        if 'tfm' in self.hparams.model_name:
            if self.config.bts_fusion:
                self.logger.info(f"bts_fusion               : {self.config.bts_fusion}")
                self.logger.info(f"bts_residual             : {bool(self.config.bts_residual)}")
            ...
            if self.config.block_scoring:
                self.logger.info(f"block size               : [{self.config.min_block_size}, {self.config.max_block_size} :: {self.config.block_stride}]")
                self.logger.info(f"conv kernel size         : {self.config.conv_kernel_size}")
                self.logger.info(f"downsample factor        : {self.config.downsample_factor}")
                self.logger.info(f"score consensus attn     : {bool(self.config.score_consensus_attn)}")
