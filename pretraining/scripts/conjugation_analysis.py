...
from token_fusing.srcs.nlu_trainer import Trainer as NLU_Trainer
from token_fusing.srcs.functions import init_random
from token_fusing.utils.base_parser import ArgsBase
from token_fusing.utils.logger import get_logger
...
def load_model(tok_type, no_context=False):
    ...
    """
    parser = ArgsBase().add_task_args()
    temp_args = parser.parse_args()

    temp = vars(temp_args)

    config = json.load(open(f"token_fusing/nlu_configs/{temp['task_name']}/config.json"))
    for arg in config:
        temp[arg] = config[arg]

    temp["tok_type"] = tok_type
    if temp["tok_type"] == 'char':
        temp["save_dir"] = "token_fusing/logs/bert-base/char_ko_2k/pretraining/128t_128b_1s_5e-05lr_42rs/ckpt"
        temp["model_name"] = "bert-base"
        temp["tok_vocab_size"] = "2k"
    elif temp["tok_type"] == 'jamo_distinct':
        temp[
            "save_dir"] = "token_fusing/logs/tfm-bert-base/jamo_distinct_ko_200/pretraining/span-character-mlm_jamo-trans3_gru_conv1-cjf_repeat_gru-up-res_128t_128b_1s_5e-05lr_42rs/ckpt"
        temp["model_name"] = "tfm-bert-base"
        ...
        if no_context:
            temp["save_dir"] = "token_fusing/logs/tfm-bert-base/jamo_distinct_ko_200/pretraining/span-character-mlm_jamo-conv1-cjf_repeat_gru-up-res_128t_128b_1s_5e-05lr_42rs/ckpt"
    ...
    prefix = ""
    if 'tfm' in args.model_name:
        if args.mlm_unit != 'token':
            prefix += f"span-{args.mlm_unit}-mlm_"
        if args.bts_fusion:
            if 'trans' in args.bts_fusion:
                temp_fusion = args.bts_fusion.replace("trans", f"trans{args.bts_trans_layer}")
            else:
                temp_fusion = args.bts_fusion
            prefix += f"bts-{temp_fusion}_"
            if args.bts_residual:
        ...
        if args.jamo_fusion:
            ...
            if args.soft_weight:
                prefix = prefix[:-1] + '-soft_'
            if args.jamo_residual:
                prefix = prefix[:-1] + "-res_"
            if args.cho_joong_first:
                prefix = prefix[:-1] + "-cjf_"
            if args.merge_vowel_first:
                prefix = prefix[:-1] + "-mvf_"
            if args.sequential_jamo_merge:
                prefix = prefix[:-1] + "-sjm_"
            if args.do_tanh:
                prefix = prefix[:-1] + "-tanh_"
            if args.gru_only_right:
                prefix = prefix[:-1] + "-gor_"
        if args.block_scoring:
            prefix += f"{args.downsample_factor}df_"
        if args.upsampling:
    ...
    args.log_dir = f"token_fusing/logs/{args.model_name}/{args.tok_name}/tasks/{args.task_name}/{prefix}{args.max_seq_len}t_{args.batch_size}b_{args.gradient_accumulation_steps}s_{args.max_epochs}e_{args.learning_rate}lr_{args.random_seed}rs"
    ...
    logger = get_logger(log_path=os.path.join(args.log_dir, "token_fusing.txt"))
...
def plot_similarity(src_vectors, tgt_vectors, tgt_tokens, tgt_words, activation=None):
    ...
    for i, ax in enumerate(grid):
        ...
        # im = ax.imshow(np.array(similarities).reshape(1, -1), cmap='RdGy_r', vmin=0, vmax=1, aspect='auto')
