...
import token_fusing.srcs.korquad_trainer as korquad_trainer
...
if temp['tok_type'] in ['jamo', 'stroke', 'cji', 'bts']:
    ...
    if temp['task_name'] == 'KorQuAD':
        ...
        temp['batch_size'] = 4
        temp['gradient_accumulation_steps'] = 4
elif temp['tok_type'] in ['morpheme']:
    if temp['task_name'] == 'KorQuAD':
        temp['max_seq_len'] = 512
        temp['doc_stride'] = 128
        temp['max_query_len'] = 128
        temp['max_answer_len'] = 60
elif temp['tok_type'] in ['subword']:
    if temp['task_name'] == 'KorQuAD':
        temp['max_seq_len'] = 512
        temp['doc_stride'] = 128
        temp['max_query_len'] = 128
        temp['max_answer_len'] = 60
elif temp['tok_type'] in ['morphemeSubword']:
    if temp['task_name'] == 'KorQuAD':
        temp['max_seq_len'] = 512
        temp['doc_stride'] = 128
        temp['max_query_len'] = 128
...
if args.task_name == 'KorQuAD':
    ...
    logging.getLogger("transformers.data.metrics.squad_metrics").setLevel(logging.WARN)  # Reduce model loading token_fusing
else:
