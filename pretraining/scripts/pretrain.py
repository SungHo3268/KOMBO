...
from token_fusing.srcs.models import CustomBertForPreTraining
from token_fusing.srcs.nlu_utils import get_bert_tokenizer, get_config
from token_fusing.srcs.nsp_dataset import TextDatasetForNextSentencePrediction
from token_fusing.srcs.functions import init_random, repeat_interleave
...
def set_parser():
    ...
    args.log_dir = f"token_fusing/logs/{args.model_name}/{args.tok_name}/pretraining/{prefix}{args.max_seq_len}t_{args.batch_size}b_{args.gradient_accumulation_steps}s_{args.learning_rate}lr_{args.random_seed}rs"
...
def set_logger(args):
    ...
    logger.info("")
    logger.info("This token_fusing inform the Running Progress of the Pre-training.\n")
...
def main(args, config, logger, trainer):
    ...
    if 'tfm' in args.model_name:
        if config.bts_fusion:
            logger.info(f"bts_fusion            : {config.bts_fusion}")
            logger.info(f"bts_residual          : {bool(config.bts_residual)}")
        ...
        if config.block_scoring:
            logger.info(f"block size            : [{config.min_block_size}, {config.max_block_size} :: {config.block_stride}]")
            logger.info(f"conv kernel size      : {config.conv_kernel_size}")
            logger.info(f"downsample factor     : {config.downsample_factor}")
            logger.info(f"score consensus attn  : {bool(config.score_consensus_attn)}")
